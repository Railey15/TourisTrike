-- Harden the tourist INSERT guard for package_bookings.
--
-- The canonical unassigned convoy state is:
--   status = 'pending'
--   booking_status = 'waiting_for_drivers'
-- and required_drivers must be a positive integer for solo and group bookings.
-- Keep accepting booking_status = 'pending' as legacy input, then normalize.
...
begin
  ...
  if tg_op = 'INSERT' then
    if auth.uid() is null
       or new.tourist_id is distinct from auth.uid() then
    ...
    if lower(
      coalesce(nullif(trim(new.status), ''), 'pending')
    ) <> 'pending' then
    ...
    if lower(
      coalesce(
        nullif(trim(new.booking_status), ''),
        'waiting_for_drivers'
      )
    ...
    if lower(
      coalesce(nullif(trim(new.booking_type), ''), 'advanced')
    ) = 'advanced' then
      if lower(
        coalesce(nullif(trim(new.payment_method), ''), '')
      ) <> 'gcash' then
        raise exception 'ADVANCED_BOOKING_REQUIRES_GCASH';
      end if;

      if new.total_amount is null
         or new.downpayment_amount is null
  ...
  -- Preserve the cancellation RPC compatibility.
  if lower(
    coalesce(
      nullif(trim(new.booking_status), ''),
