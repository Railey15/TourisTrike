...
BEGIN
  ...
  with roster as (
    ...
    union all
    select v_driver_id, 'accepted', 'assigned', 0, null::timestamptz
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'heading', loc.heading
  ) order by r.driver_id), '[]'::jsonb)
  into v_drivers
  from roster r
  left join lateral (
    select dll.latitude, dll.longitude, dll.heading
    from public.driver_live_locations dll
    join public.package_activities pa on pa.id = dll.activity_id
    where dll.driver_id = r.driver_id and pa.booking_id = v_link.booking_id
